        ...
        foreach ($result as &$item) {
            switch ($item['status']) {
                case 0:
                    ...
                    break;
                case 1:
                    $item['status'] = "Đang chuyển";
                    $item['status_css'] = 1;
                    break;
                case 2:
                    $item['status'] = "Đã chuyển";
                    $item['status_css'] = 2;
                    break;
            }
    ...
    if (!empty($result)) {
        switch ($result['status']) {
            case 0:
                ...
                break;
            case 1:
                $result['status'] = "Đang chuyển";
                break;
            case 2:
                $result['status'] = "Đã chuyển";
                break;
        }
        return $result;
    }
...
function get_waiting_transaction($start, $num_per_page)
{
    ...
    if (!empty($result)) {
        foreach ($result as &$item) {
            ...
            $item['url_edit'] = "?mod=customers&action=editTransaction&transaction={$item['transaction_code']}";
            $item['url_delete'] = "?mod=customers&action=editTransaction&transaction={$item['transaction_code']}";
            $item['url_detail'] = "?mod=customers&action=detailOrder&transaction={$item['transaction_code']}";
        }
        return $result;
    }
