...
{
    $check = db_fetch_row("SELECT * FROM `tbl_users` WHERE `reset_token` = '{$reset_token}'");
    if ($check > 0)
        return true;
    return false;
}
...
{
    $check = db_fetch_row("SELECT * FROM `tbl_users` WHERE `email` = '{$email}'");
    if ($check > 0)
        return true;
    return false;
}
...
{
    $check_user = db_fetch_row("SELECT * FROM `tbl_users` WHERE `username` = '{$username}' AND `password` = '{$password}'");
    if ($check_user > 0)
        return true;
    return false;
}
